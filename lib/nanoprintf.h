...
/* mugged by RWS, 2024-01-XX */   

// #define npf_snpr snpr
...
// RWS
#undef NPR_VISIBILITY
#define NPR_VISIBILITY
...
#include "rwsinttypes.h"
#include "rwsstdint.h"

// RWS turn off FLOAT  1 --> 0    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
int npf_fsplit_abs(float f, uint64_t *out_int_part, uint64_t *out_frac_part,
  ...
  uint32_t f_bits; { // union-cast is UB, let compiler optimize byte-copy loop.
	...
	// RWS
	unsigned i;
    for (i = 0; i < sizeof(f_bits); ++i) { dst[i] = src[i]; }
  }
  ...
  { // Convert the fractional part to base 10.
	...
	// RWS
	int i;
    for (i = 0; frac && (i < NPF_MAX_FRACTION_DEC_DIGITS); ++i) {
      frac_part *= 10;
...
#else
	    char const *s;
        for (s = cbuf; *s; ++s, ++cbuf_len); // strlen
#endif
      } break;
